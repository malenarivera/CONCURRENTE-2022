 ...
 */
public class Cuerda {
    private int cantBabuinosLadoIzqEsperando, cantBabuinosLadoDerEsperando, cantTotalBabuinosLadoIzq, cantTotalBabuinosLadoDer, limite, cantBabuinosLadoIzqPasando,cantBabuinosLadoDerPasando;
    ...
    private  void irseLadoIzq() {
        try {
            ...
            if (cantBabuinosLadoIzqPasando == 0){
                System.out.println(Thread.currentThread().getName() +" era el ultimo babuino del lado izq pasando la cuerda");
                if (cantTotalBabuinosLadoIzq >= limite) {
                    if (cantBabuinosLadoDerEsperando > 0) {
                        System.out.println(Thread.currentThread().getName() +"  se llego al limite de babuinos del lado IZQ y hay babuinos del otro lado esperando. Dejo que pasen ellos");
                        cantBabuinosLadoIzqPasando = 0;
                        ...
                        if (cantBabuinosLadoIzqEsperando > 0) {
                            System.out.println(Thread.currentThread().getName() +"  se llego al limite de babuinos del lado IZQ pero NO HAY NADIE DEL OTRO LADO y si de mi lado");
                            // pero si gatos
                            entrarBabuinosLadoIzq.release();
                        } else {
                            System.out.println(Thread.currentThread().getName() +"  se llego al limite Y NO HAY BABUINOS DE NINGUN LADO ESPERANDO");
                            // SETEAR EN 0
                    ...
                    System.out.println(Thread.currentThread().getName() +"  NO se llego al limite de la cuerda Y NO HAY MAS BABUINOS DEL LADO IZQ");
                    if (cantBabuinosLadoIzqPasando == 0) {
                        if (cantBabuinosLadoDerEsperando > 0) {
                            System.out.println(Thread.currentThread().getName() +"  NO se llego al limite de la cuerda  Y HAY BABUINOS DEL LADO DER ESPERANDO");
                            entrarBabuinosLadoIzq = new Semaphore(0);
                            entrarBabuinosLadoDer.release(limite);
                        } else {
                            // si no viene nadie
                            System.out.println(Thread.currentThread().getName() +"  NO se llego al limite de la cuerda Y NO HAY NINGUN BABUINO DE NINGUN LADO"); 
                            entrarBabuinosLadoIzq = new Semaphore(0);
                            entrarPrimerBabuino.release();
                        }
    ...
    private  void irseLadoDer() {
        try {
            ...
            if (cantBabuinosLadoDerPasando == 0){
                System.out.println(Thread.currentThread().getName() +" era el ultimo babuino del lado derecho pasando");
                if (cantTotalBabuinosLadoDer >= limite) {
                    if (cantBabuinosLadoIzqEsperando > 0) {
                        System.out.println(Thread.currentThread().getName() +"  se llego al limite de la cuerda y hay babuinos del lado IZQ esperando");
                        cantBabuinosLadoDerPasando = 0;
                        // hay gatos
                        ...
                        // no hay gatos
                        if (cantBabuinosLadoDerEsperando > 0) {
                            System.out.println(Thread.currentThread().getName() +"  se llego al limite de la cuerda pero NO HAY BABUINOS DEL LADO IZQ ESPERANDO");
                            // pero si perros
                            entrarBabuinosLadoDer.release();
                        } else {
                            System.out.println(Thread.currentThread().getName() +"  se llego al limite de la cuerda pero NO HAY BABUINOS DE NINGUN LADO" );
                            entrarBabuinosLadoDer = new Semaphore(0);
                            entrarPrimerBabuino.release();
                    ...
                    if (cantBabuinosLadoDerEsperando == 0) {
                        if (cantBabuinosLadoIzqEsperando > 0) {
                            System.out.println(Thread.currentThread().getName() +"  NO se llego al limite de la cuerda pero HAY BABUINOS DEL LADO IZQ ESPERANDO " );
                            entrarBabuinosLadoDer = new Semaphore(0);
                            entrarBabuinosLadoIzq.release(limite);
                        } else {
                            // si no viene nadie
                            System.out.println(Thread.currentThread().getName() +"  NO se llego al limite de la cuerda pero NO HAY BABUINOS DE NINGUN LADO " );
                            entrarBabuinosLadoDer = new Semaphore(0);
                            entrarPrimerBabuino.release();
                        }
